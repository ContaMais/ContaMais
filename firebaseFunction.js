                ...
                auth.createUserWithEmailAndPassword(email, senha)
                    .then((cred) => {
                        db.ref('users/' + cred.user.uid).set({
                            nome: nome,
                            email: email,
                            role: email === DONO_EMAIL ? 'dono' : 'visualizador'
                        });
                        fecharModalAuth();
        ...
        auth.onAuthStateChanged((user) => {
            if (user) {
                ...
                db.ref('users/' + user.uid).on('value', snap => {
                    const data = snap.val();
                    currentRole = (user.email === DONO_EMAIL) ? 'dono' : (data ? (data.role || 'visualizador') : 'visualizador');
                    currentUserName = data ? (data.nome || user.email.split('@')[0]) : user.email.split('@')[0];
                    aplicarPermissoes();
                });
            } else {
                currentUser = null; currentRole = 'visualizador'; currentUserName = ''; aplicarPermissoes();
            }
        });
        ...
        function aplicarPermissoes() {
            ...
            if(currentUser) {
                btnLogin.innerHTML = '🚪 <span>Sair da Conta</span>';
            } else {
                btnLogin.innerHTML = '👤 <span>Entrar / Cadastrar</span>';
            }
        ...
        function renderizarUsuariosAdmin(usersObj) {
            ...
            Object.keys(usersObj).forEach(uid => {
                ...
                if (emailsProcessados.has(u.email)) return;
                emailsProcessados.add(u.email);

                const nomeExibicao = u.nome ? `${u.nome} <br><span style="font-size:0.75rem; color:#64748b; font-weight:normal;">${u.email}</span>` : u.email;
                let selectHtml = '';
                if(u.email === DONO_EMAIL) {
                    selectHtml = `<div style="font-weight:bold; color:var(--primary); font-size:0.85rem; padding-right:5px;">👑 Dono</div>`;
                } else {
                    selectHtml = `<select onchange="db.ref('users/${uid}').update({role: this.value})" style="width:auto; padding:6px; margin:0; font-weight:bold; border-radius:6px; font-size:0.85rem; border:1px solid #ccc;">
                        <option value="contribuidor" ${u.role==='contribuidor'?'selected':''}>✏️ Contribuidor</option>
                        <option value="visualizador" ${u.role==='visualizador'?'selected':''}>👁️ Visualizador</option>
                    </select>`;
                }
                html += `<div style="padding:12px; border:1px solid var(--border-color); border-radius:8px; display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.4); margin-bottom:8px;">
                    <div style="font-weight:bold; font-size:0.9rem; color:var(--text-main); line-height:1.2;">${nomeExibicao}</div>
                    ${selectHtml}
                </div>`;
            });
            document.getElementById('lista-usuarios-admin').innerHTML = html || '<p>Nenhum usuário encontrado.</p>';
        }
        ...
        function verificarPendentes() {
            ...
            for(let pk in produtos) {
                ...
                if(p.estoque && p.estoque['_Pendente_'] && (p.estoque['_Pendente_'].f > 0 || p.estoque['_Pendente_'].u > 0)) {
                    ...
                    let totalF = Number(pend.f) || 0;
                    let totalU = Number(pend.u) || 0;

                    // Proteção de Sistema: Se a categoria diz que NÃO usa fardo (t=1), mas tem fardo preso no histórico do Firebase, converte tudo para unidade automaticamente
                    if (t === 1 && totalF > 0) {
                        totalU += totalF;
                        totalF = 0;
                    }

                    let txtQtd = t > 1 ? `${totalF} Fds e ${totalU} Un.` : `${totalU} Unidade(s)`;
                    ...
                    let inputsQtd = '';
                    if(t > 1) {
                        // Se for produto de FARDO, mostra as duas caixas: Fardo e Unidade
                        inputsQtd = `
                        <div style="display:flex; gap:8px; flex:1;">
                            <div style="flex:1;"><label style="font-size:0.7rem; color:#64748b;">Fardos (MÁX: ${totalF})</label><input type="number" id="transf-f-${pk}" value="${totalF}" min="0" max="${totalF}" style="margin:0; text-align:center; background:#000; color:var(--text-main); font-weight:bold;"></div>
                            <div style="flex:1;"><label style="font-size:0.7rem; color:#64748b;">Un. (MÁX: ${totalU})</label><input type="number" id="transf-u-${pk}" value="${totalU}" min="0" max="${totalU}" style="margin:0; text-align:center; background:#000; color:var(--text-main); font-weight:bold;"></div>
                        </div>`;
                    } else {
                        // Se for produto SÓ de UNIDADE, mostra apenas a caixa de unidade
                        inputsQtd = `
                        <div style="flex:1;"><label style="font-size:0.7rem; color:#64748b;">Unidades (MÁX: ${totalU})</label><input type="number" id="transf-u-${pk}" value="${totalU}" min="0" max="${totalU}" style="margin:0; text-align:center; background:#000; color:var(--text-main); font-weight:bold;"></div>`;
                    }
        ...
        function prepararEfetivacaoAlocacao(pk, nomeProd, maxF, maxU, t) {
            ...
            let f = elF ? parseInt(elF.value) || 0 : 0;
            let u = elU ? parseInt(elU.value) || 0 : 0;

            if(f < 0) f = 0; if(u < 0) u = 0;
            ...
            transferenciaPendente = { pk, f, u, t, areaSel, maxF, maxU };
        ...
        document.getElementById('btnConfirmTransferAction').onclick = function() {
            ...
            const { pk, f, u, t, areaSel, maxF, maxU } = transferenciaPendente;
            
            const p = produtos[pk];
            let estAtualArea = p.estoque && p.estoque[areaSel] ? p.estoque[areaSel] : {f:0, u:0};
            
            let novoAreaF = (Number(estAtualArea.f)||0) + f; 
            let novoAreaU = (Number(estAtualArea.u)||0) + u;
            if(t > 1 && novoAreaU >= t) { novoAreaF += Math.floor(novoAreaU / t); novoAreaU = novoAreaU % t; }
            
            let novoPendF = maxF - f;
            let novoPendU = maxU - u;

            let updates = {};
            updates[`produtos/${pk}/estoque/${areaSel}`] = { f: novoAreaF, u: novoAreaU };
            
            if(novoPendF <= 0 && novoPendU <= 0) {
                updates[`produtos/${pk}/estoque/_Pendente_`] = null; 
            } else {
                updates[`produtos/${pk}/estoque/_Pendente_`] = { f: novoPendF, u: novoPendU };
            }
...
document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
        ...
        db.ref('users').on('value', s => { if(currentRole === 'dono') renderizarUsuariosAdmin(s.val() || {}); });
        ...
        db.ref('produtos').on('value', s => { try { produtos = s.val() || {}; renderizarGradeAgrupada(); renderizarAreas(); verificarPendentes(); if(document.getElementById('pedidos-lista').classList.contains('active')){ const metKey = Object.keys(metodosCompra).find(k => metodosCompra[k].nome === document.getElementById('titulo-pedido-metodo').innerText.replace('Pedidos: ', '')); if(metKey) verPedidoPorMetodo(metKey); } } catch(e){} });        db.ref('categorias').on('value', s => { 
            try { 
                categorias = s.val() || {}; 
                renderizarPresets(); 
        ...
        db.ref('metodos').on('value', s => { try { metodosCompra = s.val() || {}; renderizarMetodosConfig(); renderizarPresetsMetodos(); renderizarGradeAgrupada(); renderizarPedidosMetodos(); } catch(e){} });
        let eventosTimeline = [];
        db.ref('historico_timeline').on('value', s => {
            const val = s.val();
            if(val) {
                eventosTimeline = Object.keys(val).map(k => ({ id: k, ...val[k] }));
                eventosTimeline.sort((a,b) => b.timestamp - a.timestamp); // Ordena do mais recente para o mais antigo
            } else {
                eventosTimeline = [];
            }
        ...
        db.ref('reservas').on('value', s => { try { reservasApp = s.val() || {}; if(document.getElementById('agenda-view').classList.contains('active')) renderizarCalendario(); if(document.getElementById('agenda-detalhe').classList.contains('active')) renderizarReservasDia(); if(document.getElementById('mapa-salao').classList.contains('active')) carregarReservasMapa(); } catch(e){} });
        db.ref('mapas').on('value', s => { try { mapasApp = s.val() || {}; if(document.getElementById('mapa-salao').classList.contains('active')) { carregarMapa(); carregarReservasMapa(); } } catch(e){} });
        db.ref('receitas').on('value', s => { try { receitasDB = s.val() || {}; if(document.getElementById('receitas-lista').classList.contains('active')) renderizarReceitas(); if(document.getElementById('detalhe-receita').classList.contains('active')) verDetalhesReceita(receitaAtualKey); } catch(e){} });
...
function moverCategoriaAreaEdit(cat, direcao) {
    ...
    atualizarOrdemCategoriasAreaEdit();
        ...
        function renderizarTotalLinhaDupla(total, tam) { 
            const tf = tam > 1 ? tam : 1; 
            if (tam > 1) { return `<div class="total-container"><span class="total-fardos">${Math.floor(total/tf)} Fardos</span><span class="total-unidades">${total%tf} Un. Soltas</span></div>`; } 
            else { return `<div class="total-container"><span class="total-fardos" style="color:var(--primary); font-size:1.05rem;">${total} Un.</span></div>`; }
        }
        ...
        function verGrupo(key, tipo) { 
            try {
                ...
                document.getElementById('tabela-produtos-filtrada').innerHTML = prodsFiltrados.map(k => { 
                    ...
                    const tot = Object.values(p.estoque||{}).reduce((a,v)=>typeof v==='object'&&v!==null?a+((Number(v.f)||0)*t)+(Number(v.u)||0):a+Number(v||0),0); 
...
</span></div></td><td style="text-align:right">${renderizarTotalLinhaDupla(tot,t)}</td></tr>`; 
                }).join(''); 
...
const ordemSubgrupos = Object.keys(subgruposDaCategoria).sort((a, b) => {
    ...
    return a.localeCompare(b);
            ...
            ordemSubgrupos.forEach(subKey => {
                ...
                const qtdF = Number(est.f) || 0;
                const qtdU = Number(est.u) || 0;
                ...
                let viewFardo = modoContagemAtivo 
                    ? (t > 1 ? `<div class="qty-box" style="border: 1px solid var(--primary); background: #000; width: 75px;">
                         <input type="number" class="qty-input" id="cont-f-${pk}" value="" style="background: #000; color: #fff;">
                    ...
                    : `<span style="font-size:1.15rem; font-weight:800; color:var(--text-main);">${t > 1 ? qtdF : '-'}</span>`;

                let viewUnidade = modoContagemAtivo 
                    ? `<div class="qty-box" style="border: 1px solid var(--primary); background: #000; width: 75px;">
                         <input type="number" class="qty-input" id="cont-u-${pk}" value="" style="background: #000; color: #fff;">
                         <div class="qty-ctrls" style="border-left: 1px solid var(--primary);">
                             <button class="qty-btn-up" onclick="document.getElementById('cont-u-${pk}').value=(Number(document.getElementById('cont-u-${pk}').value)||0)+1">▲</button>
                             <button class="qty-btn-down" onclick="document.getElementById('cont-u-${pk}').value=Math.max(0,(Number(document.getElementById('cont-u-${pk}').value)||0)-1)">▼</button>
                         </div>
                       </div>`
                    : `<span style="font-size:1.15rem; font-weight:800; color:var(--text-main);">${qtdU}</span>`;

                let txtTopTotal = t > 1 ? `${Math.floor(totalUnidades / t)} Fds | ${totalUnidades % t} Un.` : `${totalUnidades} Un.`;
                    ...
                    : `<td style="text-align:right; border-left: 1px solid rgba(255,255,255,0.05);">
                         <div style="display:flex; flex-direction:column; line-height:1.2;">
                           ...
                           ${t > 1 ? `<span style="font-size:0.75rem; color:#64748b; font-weight:700;">${totalUnidades} Un. Totais</span>` : ''}
                                ...
                                lista.innerHTML += `
                    <tr>
                        <td style="font-size: 0.95rem; font-weight:700;">${p.nome}</td>
        ...
        function attEst(pk, ar, tam) { 
            ...
            const f = elF ? (Number(elF.value) || 0) : 0; 
            const u = Number(document.getElementById(`u-${pk}`).value) || 0; 
            db.ref(`produtos/${pk}/estoque/${ar}`).set({f,u}); 
            document.getElementById(`tot-cell-${pk}`).innerHTML = renderizarTotalLinhaDupla(f*tam+u,tam); 
        }
        ...
        function verDetalhesProduto(key) { 
            try {
                ...
                let tG = 0; const locaisHtml = (p.locais||[]).map(loc => { const a = areas.find(x => x.nome === loc); const v = p.estoque?.[loc] || {f:0,u:0}; const q = (v.f||0)*t + (v.u||0); tG += q; return `<tr><td><div style="display:flex; align-items:center; gap:8px;"><button class="action-icon info-btn" onclick="verArea('${a?.key}', '${loc}')">ⓘ</button><span>${loc}</span></div></td><td style="text-align:right">${renderizarTotalLinhaDupla(q,t)}</td></tr>`; }).join(''); 
                ...
                const boxFardosGeral = t > 1 ? `<div><strong style="color:var(--primary);">Fardos Fechados:</strong> <span style="color:#64748b;">${Math.floor(tG/t)} Fardos (Geral)</span></div>` : '';
                
                // Formatação Preço de Custo Geral (BLINDADO COM REQ-DONO)
                let htmlCustoGeral = `<p class="req-dono" style="font-size:0.9rem; margin-bottom:5px;"><strong>Preço de Custo (Geral):</strong> R$ ${Number(p.precoCusto||0).toFixed(2).replace('.', ',')}</p>`;
                
                // Formatação Destilados Específico (BLINDADO COM REQ-DONO)
                ...
                if(p.detalhesDestilado && p.detalhesDestilado.ml) {
                    ...
                    htmlInfoDestilado = `<div class="req-dono"><hr style="opacity:0.2; margin: 10px 0;">
                ...
                document.getElementById('info-extra-prod').innerHTML = `
                    <div style="padding:12px; background:var(--row-even); border-radius:8px; border:1px solid var(--border-color); display:flex; flex-direction:column; gap:8px; margin-top:10px;">
                        ${htmlCustoGeral}
                        ${htmlInfoDestilado}
                        <hr class="req-dono" style="opacity:0.2; margin: 5px 0;">
        ...
        function salvarMetodo() { const id=document.getElementById('edit-met-id').value, n=document.getElementById('met-nome').value; if(n){ if(id){ const old=metodosCompra[id].nome; db.ref('metodos/'+id).update({nome:n}).then(()=>{if(old!==n)Object.keys(produtos).forEach(pk=>{if(produtos[pk]&&produtos[pk].metodoCompra===old)db.ref('produtos/'+pk).update({metodoCompra:n});});}); }else db.ref('metodos').push({nome:n}); resetarFormMetodo(); } }
...
function moverCatSub(index, direcao) {
    ...
    renderCatSubs();
        ...
        function salvarCategoria() { 
            ...
            if(n && sel.length){ 
                const d = { nome:n, areas:sel, unidadesFardo:t, subgrupos: hasSub ? currentCatSubgrupos : [] }; 
                if(id) { 
        ...
        function abrirConfigCategoria(k) {
            ...
            let htmlInfo = `<p><strong>Unidades por Fardo:</strong> ${c.unidadesFardo}</p><p><strong>Áreas Armazenadas:</strong> ${(c.areas||[]).join(', ') || 'Nenhuma'}</p>`;
            ...
            grid.innerHTML = chaves.map(k => {
                ...
                <div class="card-nav" onclick="verDetalhesReceita('${k}')" style="display:flex !important; align-items:center; gap:12px; padding: 15px; flex-direction: row; justify-content:flex-start;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:4px; text-align:left;">
                        ...
                        <span class="req-dono" style="font-size:0.85rem; color:#10b981; font-weight:800; display:block !important;">R$ ${custoTotal.toFixed(2).replace('.', ',')}</span>
                    </div>
                </div>`;
            }).join('');
        }
        ...
        function verDetalhesReceita(k) {
            ...
            let ingredientesHtml = (r.ingredientes || []).map(ing => {
                ...
                return `<li style="margin-bottom:8px; color:var(--text-main); font-size:0.95rem; list-style:none; border-bottom:1px solid rgba(251, 238, 227, 0.1); padding-bottom:8px; display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <span class="req-dono" style="color:#10b981; font-weight:800; font-size:0.85rem;">R$ ${custoIng.toFixed(2).replace('.', ',')}</span>
                        </li>`;
            }).join('');
            ...
            container.innerHTML = `
                <div style="display:flex; gap:15px; align-items:flex-start;">
                    <div style="flex:1;">
                        <div id="receita-info-painel" class="req-dono" style="background:rgba(16, 185, 129, 0.1); border:1px solid #10b981; border-radius:12px; padding:15px; margin-bottom:15px;">
