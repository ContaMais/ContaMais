window.addEventListener('load', function() {
        ...
        function abrirDashboard() {
            ...
            Object.keys(produtos).forEach(pk => {
                ...
                const custoUnidade = Number(p.precoCusto) || 0;
                let t = 1; 
                ...
                Object.keys(p.estoque || {}).forEach(loc => {
                    ...
                    const qtdTotal = (Number(v.f)||0)*t + (Number(v.u)||0);
                    
                    if (qtdTotal > 0) { // Só entra na conta se tiver algum item físico
                        const valorNaArea = qtdTotal * custoUnidade;
                        
                        if(custosPorArea[loc] !== undefined) {
                            custosPorArea[loc] += valorNaArea;
                            totalGeral += valorNaArea;
                            
                            // Guarda o cálculo mastigado para a gaveta de informações
                            detalhesPorArea[loc].push({
                                id: pk,
                                nome: p.nome,
                                f: Math.floor(qtdTotal / t),
                                u: qtdTotal % t,
                                t: t,
                                valor: valorNaArea
                            });
            ...
            const htmlGrid = areas.filter(a => a.nome).map(a => {
                ...
                let detalhesHtml = detalhesPorArea[loc].map(d => {
                    let txtQtd = d.t > 1 ? `${d.f} Fds e ${d.u} Un.` : `${d.u} Un.`;
...
function salvarContagemAuditoria() {
    ...
    inputsUnidade.forEach(inputU => {
        ...
        let f = inputF && inputF.value !== "" ? parseInt(inputF.value) : 0;
        let u = inputU.value !== "" ? parseInt(inputU.value) : 0;
